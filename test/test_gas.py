...
class TestImperfectGas(TestCase):
    def test_all(self):
        from pyavia.aero import ImperfectGas
        ...
        gas = ImperfectGas(h=dim(1.937741828959338, 'MJ/kg'), P=dim(1, 'bar'),
                           M=0.0, gas='air')
        ...
        inlet_flow = ImperfectGas(P=dim(1, 'atm'), T=dim(288.15, 'K'), M=0.0,
                                  gas='air')
        h_in = inlet_flow.h.convert('kJ/kg').value
        s_in = inlet_flow.s.convert('kJ/kg/K').value

        isen_exit = ImperfectGas(P=inlet_flow.P * 20, T=dim(667.36, 'K'), M=0.0)
        ...
        act_exit = ImperfectGas(P=inlet_flow.P * 20, T=dim(731.42, 'K'), M=0.0)
        ...
        from_stag = ImperfectGas(P0=dim(1895.2, 'psf'), P=dim(242.215, 'psf'),
                                 T0=dim(701.95, '°R'))
...
class TestPerfectGas(TestCase):
    def test_all(self):
        from pyavia.aero import PerfectGas
        from pyavia.units import dim
