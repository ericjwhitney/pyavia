...
# TODO Roll anything original here into new test_atmosphere.

class TestAtmosphere(TestCase):
    # noinspection PyUnresolvedReferences
    def test__init__(self):
        from pyavia.aerodynamics import Atmosphere_OLD
        from pyavia.units import Dim

        # Check invalid constructions.
        with self.assertRaises(TypeError):
            # noinspection PyTypeChecker,PyArgumentList
            Atmosphere_OLD(3)  # Invalid postional arg.
            Atmosphere_OLD(h=Dim(1000, 'ft'))  # Invalid combination.

        with self.assertRaises(ValueError):
            Atmosphere_OLD(H=0)  # Arg needs dimensions.

        # test for correct results from some standard altitude values.
        Atmosphere_OLD.set_default_style('SI')  # K, kPa, m, etc.

        atm = Atmosphere_OLD(H='SSL')  # Sea level - thorough test.
        ...
        atm = Atmosphere_OLD(H_press=Dim(48000, 'ft'), T=Dim(-52, '°C'))
        ...
        atm = Atmosphere_OLD(H_press=Dim(7000, 'ft'), T_offset=Dim(5, 'Δ°C'))
        self.assertAlmostEqual(atm.density.convert('slug/ft^3').value,
        ...
        atm = Atmosphere_OLD(h_geometric=Dim(50120.16, 'ft'))
