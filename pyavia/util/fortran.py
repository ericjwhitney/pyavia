...
# Establish two-way mapping between Fortran and Numpy types.  All names
# are converted to lowercase.

# Mapping from Fortran -> NumPy types.
_TYPEMAP_F2NP = {
    'logical*1': 'bool_',
    'logical': 'bool_',  # LOGICAL Default.
    'complex*32': 'complex64',  # Note: Fortran specifies size ...
    'complex*64': 'complex128',  # ... of each part in complex.
...
# Mapping from NumPy -> Fortran types.  Note:
#   If multiple Fortran types might match the given NumPy type (e.g.
#   'float64' -> 'real*8' or 'real'), this mapping is arranged so that
#   the longer definition (e.g. 'real*8') is returned.
_TYPEMAP_NP2F = {
    'bool_': 'logical*1',
    'complex64': 'complex*32',  # Fortran specifies size ...
    'complex128': 'complex*64',  # ... of each part in complex.
    'int8': 'integer*1',
    'int16': 'integer*2',
    'int32': 'integer*4',
    'int64': 'integer*8',
...
def fortran_array(arr, dtype=None, *, copy=True, order='F', subok=False,
                  ndmin=1, ftype=None) -> NDArray:
    """
    Return a new FortranArray object using identical arguments to
    `np.array()`, with the following diffSerences:

        - Fortran type `ftype` (case insensitive) may be given instead
          of `dtype` (but not both).  See `FortranArray.__new__` for
          types.
    ...
    if ftype:
        try:
            dtype = _TYPEMAP_F2NP[ftype.casefold()]
        except KeyError:
...
def fortran_do(start: int, stop: int, step: int = 1) -> range:
    """
    Returns sequence in a similar style to ``range()``, but gives the
    same values as the Fortran style do-loop construct.  Differences
    are:
    ...
    As a more complete example, we can directly implement a
    Fortran-style algorithm in Python.  First we declare the matricies
    and arrays:
...
class FortranArray(np.ndarray):
    """
    Numpy array subclass emulating a Fortran-style index-1 array that
    can be used directly when implementing algorithms from Fortran or
    Matlab.  Indexing supports slice-off-the-end which is valid in
    Fortran 90 and NumPy.
    ...
    Behaviour matches `np.ndarray` except:
        ...
        - The additional `FortranArray.ftype` property returns the
          Fortran equivalent of the underlying Numpy data type in use.
    ...
    INIT_DEFAULT = 0  # Init. value for directly constr. FortranArray.
    ...
    def __new__(cls, *dims, ftype='real*8'):
        ...
        -------
        result : FortranArray
            Subclass of `np.ndarray` of type `ftype`, using Fortran
            ordering, with all values initialised to `INIT_DEFAULT`.
        ...
        if not dims:
            raise AttributeError("At least one array dimension must be "
                                 "provided.")
        try:
            np_type = _TYPEMAP_F2NP[ftype.casefold()]
        except KeyError:
            raise ValueError(f"Fortran type '{ftype}' not supported.")
    ...
    def ftype(self):
        ...
        """
        # If the Fortran <-> NumPy mapping is complete, this lookup
        # should never fail.
        return _TYPEMAP_NP2F[str(self.dtype)]

    # -- Private Methods -----------------------------------------------

    def _f2py_idx(self, fort_idx):
        """
        Convert Fortran-style array slices (index-1) or individual
        indices into Python / C equivalent (index-0).
        """
        ...
        for ax_idx, ax_dim in zip(fort_idx, self.shape):
            if isinstance(ax_idx, slice):
                ...
                if 0 <= py_start and (py_stop is None or 0 <= py_stop):
                    ...
                    raise IndexError(f"Invalid Fortran-style array "
                                     f"slice: {ax_idx}")

            else:
                if not 1 <= ax_idx <= ax_dim:
                    raise IndexError(f"Fortran-style index out of "
                                     f"range: {fort_idx}.")
