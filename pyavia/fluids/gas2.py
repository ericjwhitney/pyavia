...
Revised verion to make gases into frozen dataclasses.
...
class Gas(ABC):
    ...
    after initialisation - they simply allow access to all the separate
    properties of the gas, which are fixed.
    """

    def __init__(self, *, R: Dim, m_dot: Dim = dim(0.0, 'kg/s'), FAR: float = 0.0):
        """
        Parameters
        ----------
        R : Dim
        ...
        """
        self._R, self._m_dot, self._FAR = R, m_dot, FAR
    ...
    def a(self) -> float:
        ...
        return (self._R * self.T * self.γ) ** 0.5
    ...
    def Q(self) -> float:
        r"""
        Enthalpy flowrate :math:`Q = h * \dot{m}`. 
        """
    ...
    def R(self) -> float:
        """
        Gas constant R for the specific gas. :math:`R_{spec} = R_{univ} / M`
...
def _solve_TPM(flow: Type[Gas], *, init_gas: Gas = None,
    ...
    for name in unknown_vars:
        ...
        # TODO temp fix, this will go in the longer term.
        if isinstance(var_ref, Dim):
