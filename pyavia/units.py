...
__all__ = ['OUTPUT_UCODE_PWR', 'STD_UNIT_SYSTEM', 'CACHE_COMPUTED_CONVS',
           'CACHE_MADE_UNITS', 'CONV_PATH_LENGTH_WARNING', 'DimValueType',
           'DimOpType', 'Dim', 'add_base_unit', 'add_unit', 'convert', 'dim',
           'is_dimoptype', 'set_conversion', 'str2dim', 'to_absolute_temp',
           'units_aware']
...
ConvFactorType = Union[DimScalarType, Fraction]
...
class Dim(NamedTuple):
    ...
    def __round__(self, n=None) -> Dim:
        return Dim(round(self.value, n), self.units)

    # -- Binary Operators -----------------------------------------------------

    def __add__(self, rhs: DimOpType) -> DimOpType:
        r"""
    ...
    def __sub__(self, rhs: DimOpType) -> DimOpType:
        r"""
    ...
    def __mul__(self, rhs: DimOpType) -> DimOpType:
        """
    ...
    def __truediv__(self, rhs: DimOpType) -> DimOpType:
        """
    ...
    def __matmul__(self, rhs: DimOpType) -> DimOpType:
        """
    ...
    def __pow__(self, pwr: Union[int, float]) -> DimOpType:
        """
    ...
    def __radd__(self, lhs: DimOpType) -> DimOpType:
        """See ``__add__`` for addition rules."""
        return Dim(lhs, '') + self

    def __rsub__(self, lhs: DimOpType) -> DimOpType:
        """See ``__sub__`` for subtraction rules."""
        return Dim(lhs, '') - self

    def __rmul__(self, lhs: DimOpType) -> DimOpType:
        """See ``__mul__`` for multiplication rules."""
        return Dim(lhs, '') * self

    def __rtruediv__(self, lhs: DimOpType) -> DimOpType:
    ...
    def to_value(self, to_units: str = None) -> DimValueType:
        """
    ...
    def to_value_sys(self, unit_system: str = None) -> DimValueType:
        """
...
def convert(value: DimValueType,
            from_units: str, to_units: str) -> DimValueType:
    """
...
def dim(value: Union[DimOpType, str] = 1, units: str = None) -> Dim:
    """
...
def set_conversion(from_label: str, to_label: str, *, fwd: ConvFactorType,
    ...
    rev : DimValueType or str (optional)
...
_CONVERSIONS = WtDirGraph()  # Dfined conversions between Units().
...
def _convert(value: DimValueType,
...
def _convert_total_temp(x: DimValueType,
                        from_unit: str, to_unit: str) -> DimValueType:
    """
...
def _dim_mul_generic(lhs: Dim, rhs: DimOpType, mult_op: Callable) -> DimOpType:
    """
...
# =============================================================================
# Return status of aching for made units after adding derived types.
