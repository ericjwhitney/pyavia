...
def hole_scf_3d(rt: float, bt: float, zt: float, rw: float,
    ...
    bt : float
        Straight shank length to plate thickness ratio, :math:`0 \leq b/t
        \leq 1`.  If ``bt == 1`` the hole is straight shank only.
    zt : float
        Location where stress concentration factor is required as a ratio of
    ...
    scf : float
        ...
        Depending on load case:
            ...
            - Remote bending: :math:`K_b=\sigma_{max}/[6M/(t^2)]` where `M` =
              remote applied moment per unit width.
            - Pin (or rivet) load: :math:`K_p=\sigma_{max}/[P/(2rt)]` where
              `P` = pin load.
            - Wedge load: :math:`K_w=\sigma_{max}/[P/(2rt)]` where `P` = wedge
              load

            .. note:: `K` / SCF for simulated rivet loading is obtained by
               adding one-half of the SCF for remote tension S = p/(2wt) and
               one-half of the SCF for wedge loading (`2w` is total width of
               plate).
    """
    if not (0.0 <= bt <= 1.0):
...
def _scf_csunk(rt, k, zt, lcase):
    ...
    value of k = 0, 1, 2 or 3 corresponding to b/t = 0, 0.25, 0.50 or 0.75.
    Results for other b/t values are computed by linear interpolation
    between the bracketing b/t values.
